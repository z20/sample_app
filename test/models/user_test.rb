...
class UserTest < ActiveSupport::TestCase
 ...
 test "email validation should reject invalid addresses" do
  ...
  test "asssociated microposts should be destroyed" do
    @user.save
    @user.microposts.create!(content: "Lorum ipsum")
  ...
  test "should follow and unfollow a user" do
    zach = users(:zach)
    archer  = users(:archer)
    assert_not zach.following?(archer)
    zach.follow(archer)
    assert zach.following?(archer)
#    assert archer.followers.include?(zach)
    zach.unfollow(archer)
    assert_not zach.following?(archer)
  end
